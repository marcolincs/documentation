...
IBM&reg; offers access to a wide variety of quantum processing units (QPUs). All QPUs deployed by IBM are based on superconducting qubit technology, as the control and scalability of this technology pave a clear path to achieving quantum advantage with these QPUs. You can see the details of all public IBM QPUs on the [Compute resources](https://quantum.cloud.ibm.com/computers) page.
...
In addition to Find the calibration data on a details card for a QPU (found by selecting a QPU on the [Compute resources](https://quantum.cloud.ibm.com/computers) page), below the Details section. Click the arrow to display the view options: map, graph, or table view. 
